...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
evidence = repmat(A, d, 1);
evidence(:, V) = repmat([1:d]', 1, length(V));

req_factors = F(unique([G.var2factors{V}]));



for k = 1:length(req_factors),
	LogBS += log(GetValueOfAssignment(req_factors(k), evidence(:, req_factors(k).var)));
end;
